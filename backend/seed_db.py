...
def seed():
    ...
    try:
        # Create user
        user = User(
            email="demo@learning.agent", hashed_password=get_password_hash("demo123")
        )
        db.add(user)
        db.flush()
        ...
        for milestone_data in plan_data["milestones"]:
            milestone = Milestone(
                ...
                + timedelta(weeks=milestone_data["target_week"]),
                status=(
                    MilestoneStatus.ACTIVE
                    if milestone_data["target_week"] <= 2
            ...
            for task_data in milestone_data["tasks"]:
                task = Task(
                    ...
                    status=(
                        ...
                        if "Quiz" in task_data["title"]
                        and "Core" in milestone_data["title"]
