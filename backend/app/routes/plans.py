...
async def create_plan(plan_create: PlanCreate, db: Session = Depends(get_db)):
    ...
    goal = (
        db.query(Goal)
        .filter(
            Goal.id == plan_create.goal_id, Goal.user_id == 1  # TODO: Get from auth
        )
        .first()
    )
    ...
    db.query(Plan).filter(
        Plan.goal_id == plan_create.goal_id, Plan.status == PlanStatus.ACTIVE
...
async def get_plans_overview(db: Session = Depends(get_db)):
    ...
    active_plans = (
        db.query(Plan)
        .filter(Plan.status == PlanStatus.ACTIVE, Plan.goal_id == Goal.id)
        .join(Goal)
    ...
    for plan in active_plans:
        ...
        tasks = (
            db.query(Task).join(Milestone).filter(Milestone.plan_id == plan.id).all()
        )
...
async def get_active_plan_milestones(
    plan_id: Optional[int] = None, db: Session = Depends(get_db)
):
    ...
    milestones = (
        db.query(Milestone)
        .filter(Milestone.plan_id == active_plan.id)
...
async def get_active_plan_revisions(
    plan_id: Optional[int] = None, db: Session = Depends(get_db)
):
...
async def get_plan_milestones(plan_id: int, db: Session = Depends(get_db)):
    ...
    milestones = (
        db.query(Milestone)
        .filter(Milestone.plan_id == plan_id)
        .order_by(Milestone.order)
        .all()
    )
    return milestones
...
async def get_dashboard_stats(
    ...
    tasks = (
        db.query(Task).join(Milestone).filter(Milestone.plan_id == active_plan.id).all()
    )
    ...
    completed_tasks = (
        ...
        .join(Milestone)
        .filter(
            Milestone.plan_id == active_plan.id, Task.status == TaskStatus.COMPLETED
        )
        .count()
    )
    ...
    # Time spent
    time_spent = (
        db.query(func.sum(ActivityLog.time_spent_min))
    ...
    dates_query = (
        db.query(func.date(ActivityLog.timestamp))
        .filter(ActivityLog.user_id == user_id)
        .distinct()
        .order_by(func.date(ActivityLog.timestamp).desc())
        .all()
    )
