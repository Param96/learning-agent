...
async def revise_plan_endpoint(
    plan_id: int, trigger: str, trigger_data: dict, db: Session = Depends(get_db)
):
...
async def get_plan_diff(plan_id: int, revision_id: int, db: Session = Depends(get_db)):
    ...
    revision = (
        db.query(PlanRevision)
        .filter(PlanRevision.id == revision_id, PlanRevision.plan_id == plan_id)
        .first()
    )
    ...
    prev_version = (
        db.query(Plan)
        .filter(
            Plan.goal_id == revision.plan_id, Plan.version == revision.plan.version - 1
        )
        .first()
    )
