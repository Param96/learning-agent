...
async def submit_attempt(attempt_create: AttemptCreate, db: Session = Depends(get_db)):
    ...
    if attempt.score is not None and attempt.score < (
        settings.QUIZ_FAILURE_THRESHOLD * 100
    ):
...
async def _create_quiz_failure_nudge(attempt: Attempt, task: Task, db: Session):
    ...
    nudge = Nudge(
        ...
        message=(
            nudge_response.message
            if nudge_response
            else f"You scored {attempt.score}% on {task.title}. Let's review the material."
        ),
        acted_on=False,
    )
...
async def get_nudges(db: Session = Depends(get_db)):
    ...
    nudges = (
        db.query(Nudge)
        .filter(Nudge.user_id == 1)  # TODO: Get from auth
