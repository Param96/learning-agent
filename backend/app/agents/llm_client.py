...
if base_url:
    ...
    # Strip again just in case they had `.../chat/completions/`
    if base_url.endswith("/"):
        base_url = base_url[:-1]

client = AsyncOpenAI(
    base_url=base_url, api_key=settings.LLM_API_KEY or "dummy-key", timeout=60.0
)
...
async def call_gemini(
    ...
    try:
        ...
        try:
            ...
            print(
                f"Primary model ({settings.LLM_MODEL}) failed: {primary_err}. Trying fallback model..."
            )
            completion = await client.chat.completions.create(
